...
class MonitorTrcDepositsCommand extends Command
{
    /**
        ...
        foreach ($wallets as $wallet) {
            ...
            if($response->successful() && isset($data['total']) && $data['total'] > 0) {
                ...
                foreach ($data['data'] as $transaction) {
                    ...
                    $currency = (new CurrencyRepository())->getCurrencyByName($transaction['tokenName']);

                    if(!$currency) continue;

                    $deposit = Deposit::where('txn', $transaction['transactionHash'])->where('network_id', NETWORK_TRC)->first();
                    ...
                    $data = [
                        ...
                        'confirms' => 1,
                        'amount' => math_divide($transaction['amount'], (string)pow(10, $transaction['decimals'])),
                        'full_amount' => $transaction['amount'],
                    ];

                    $service->handleDeposit('trc20', $data);
                }
